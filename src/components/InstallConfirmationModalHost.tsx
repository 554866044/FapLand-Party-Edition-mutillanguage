...
export function InstallConfirmationModalHost() {
    ...
    const fileName = confirmation.analysis.filePath.split(/[/\\]/).pop() ?? "Unknown File";
    const contentName = confirmation.analysis.contentName;

    return (
        <div className="fixed inset-0 z-[130] flex items-center justify-center bg-black/70 px-4" role="dialog" aria-modal="true" aria-labelledby="install-confirm-modal-title">
            <div className="w-full max-w-lg rounded-[1.5rem] border border-white/10 bg-zinc-950 p-6 text-white shadow-2xl">
                <div className="flex flex-col items-center text-center">
                    ...
                    <div className="mt-4 w-full rounded-2xl border border-white/5 bg-white/5 p-4 text-left">
                        <div className="text-[11px] font-semibold uppercase tracking-[0.2em] text-zinc-500">File Name</div>
                        <div className="mt-1 font-medium text-zinc-100 break-all">{fileName}</div>

                        <div className="mt-4 text-[11px] font-semibold uppercase tracking-[0.2em] text-zinc-500">Internal Name</div>

...
vi.mock("./webVideo", () => ({
  buildWebsiteVideoProxyUri: vi.fn(
    (videoUri: string) => `app://external/web-url?target=${encodeURIComponent(videoUri)}`
  ),
...
describe("playableVideo", () => {
  beforeEach(() => {
    ...
    vi.mocked(isStashProxyUri).mockImplementation((uri: string) =>
      uri.startsWith("app://external/stash?")
    );
  ...
  it("returns raw website urls while caching warms in the background", async () => {
    vi.mocked(isWebsiteVideoResolvableUri).mockReturnValue(true);
    vi.mocked(warmWebsiteVideoCache).mockReturnValue(
      Promise.resolve({
    ...
    const result = await resolvePlayableVideoUri(
  ...
  it("returns the cached local website video when one already exists", async () => {
    vi.mocked(isWebsiteVideoResolvableUri).mockImplementation((uri) =>
      uri.startsWith("app://external/web-url?")
    );
    ...
    vi.mocked(runCommand).mockImplementation(async (_command, args) => {
      if (args.includes("-show_entries")) {
        return {
          stdout: Buffer.from(
            JSON.stringify({ streams: [{ codec_name: "h264", pix_fmt: "yuv420p" }] }),
            "utf8"
          ),
          stderr: Buffer.alloc(0),
        };
  ...
  it("resolves local uris and transcodes on cache miss", async () => {
    ...
    vi.mocked(fs.stat).mockImplementation(async (targetPath?: unknown) => {
      ...
      if (
        ...
        targetPath.includes("video-playback-cache") &&
        !outputExists
      ) {
    ...
    vi.mocked(runCommand).mockImplementation(async (_command, args) => {
      if (args.includes("-show_entries")) {
        return {
          stdout: Buffer.from(
            JSON.stringify({ streams: [{ codec_name: "hevc", pix_fmt: "yuv420p" }] }),
            "utf8"
          ),
          stderr: Buffer.alloc(0),
        };
  ...
  it("retranscodes when source fingerprint changes", async () => {
    ...
    const ffmpegCalls = vi
      .mocked(runCommand)
      .mock.calls.filter(([, args]) => !args.includes("-show_entries"));
  ...
  it("transcodes H.264 with 10-bit pixel format (yuv420p10le) as Chromium cannot decode it", async () => {
    ...
    vi.mocked(fs.stat).mockImplementation(async (targetPath?: unknown) => {
      ...
      return { isFile: () => true, size: 1000, mtimeMs: 2000 } as ReturnType<
        typeof fs.stat
    ...
    vi.mocked(runCommand).mockImplementation(async (_command, args) => {
      if (args.includes("-show_entries")) {
        return {
          ...
          stdout: Buffer.from(
            JSON.stringify({ streams: [{ codec_name: "h264", pix_fmt: "yuv420p10le" }] }),
            "utf8"
          ),
          stderr: Buffer.alloc(0),
        };
  ...
  it("repairs a Stash proxy URI missing sourceId", async () => {
    const { isWebsiteVideoResolvableUri, isStashProxyUri, parseStashProxyUri } =
      await import("./webVideo");
    vi.mocked(isWebsiteVideoResolvableUri).mockReturnValue(true);
    vi.mocked(isStashProxyUri).mockReturnValue(true);
    vi.mocked(parseStashProxyUri).mockReturnValue({
      targetUrl: "http://localhost:9999/scene/123/stream",
    });

    const partialUri =
      "app://external/stash?target=http%3A%2F%2Flocalhost%3A9999%2Fscene%2F123%2Fstream";
    const fullUri =
      "app://external/stash?sourceId=source-123&purpose=video&target=http%3A%2F%2Flocalhost%3A9999%2Fscene%2F123%2Fstream";

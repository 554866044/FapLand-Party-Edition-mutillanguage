...
export const round = sqliteTable('Round', {
    ...
    type: text('type', { enum: ['Normal', 'Interjection', 'Cum'] }).notNull().default('Normal'),
...
export const playlistTrackPlay = sqliteTable('PlaylistTrackPlay', {
    id: text('id').primaryKey().$defaultFn(() => createId()),
    playlistId: text('playlistId').notNull().references(() => playlist.id, { onDelete: 'cascade', onUpdate: 'cascade' }),
    roundId: text('roundId').notNull(),
    nodeId: text('nodeId'),
    poolId: text('poolId'),
    playedAt: integer('playedAt', { mode: 'timestamp' }).notNull().$defaultFn(() => new Date()),
}, (table) => ({
    playlistPlayedAtIdx: index('PlaylistTrackPlay_playlistId_playedAt_idx').on(table.playlistId, table.playedAt),
    playlistPoolRoundIdx: index('PlaylistTrackPlay_playlistId_poolId_roundId_idx').on(table.playlistId, table.poolId, table.roundId),
}));
...
export const singlePlayerRunHistory = sqliteTable('SinglePlayerRunHistory', {
    id: text('id').primaryKey().$defaultFn(() => createId()),

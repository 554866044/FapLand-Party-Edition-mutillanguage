...
const SPRITE_SCREENSHOT_WIDTH = 64;
...
function decodeBmpFrame(buffer: Buffer): DecodedFrame {
  ...
  const rowStride = bitsPerPixel === 24 ? (width * bytesPerPixel + 3) & ~3 : width * bytesPerPixel;
...
const dct64 = [
  ...
  1.6351696263031674, 1.5766928552532127, 1.5144176930129691, 1.448494165902934, 1.3790810894741339,
  1.3063456859075537, 1.2304631811612539, 1.151616382835691, 1.0699952397741948, 0.9857963844595683,
  0.8992226593092132, 0.8104826280099796, 0.7197900730699766, 0.627363480797783, 0.5334255149497968,
  0.43820248031373954, 0.3419237775206027, 0.24482135039843256, 0.1471291271993349,
  0.049082457045824535,
];

const dct32 = [
  1.9975909124103448, 1.978353019929562, 1.9400625063890882, 1.8830881303660416, 1.8079785862468867,
  1.7154572200005442, 1.6064150629612899, 1.4819022507099182, 1.3431179096940369, 1.191398608984867,
  1.0282054883864435, 0.8551101868605644, 0.6737797067844401, 0.48596035980652796,
  0.2934609489107235, 0.09813534865483627,
];

const dct16 = [
  1.9903694533443936, 1.9138806714644176, 1.76384252869671, 1.546020906725474, 1.2687865683272912,
  0.9427934736519956, 0.5805693545089246, 0.19603428065912154,
];
...
function generateSpritePhashHex(sprite: DecodedFrame): string {
...
async function extractSpriteBmp(
  ffmpegPath: string,
  videoPath: string,
...
if (!parentPort) {
  console.error("[PhashWorker] Started without parentPort on process.");
  process.exit(1);
}

function sendLog(message: string, isError = false) {
  parentPort?.postMessage({
    type: "worker-log",
    payload: { message: `[PhashWorker] ${message}`, isError },
  });
}

process.on("uncaughtException", (error) => {
  sendLog(`Uncaught exception: ${error.message}\n${error.stack ?? ""}`, true);
  parentPort?.postMessage({
    type: "phash-error",
    taskId: "unknown",
    payload: { message: `Uncaught exception: ${error.message}`, stack: error.stack },
  });
});
...
process.on("unhandledRejection", (reason) => {
  ...
  sendLog(`Unhandled rejection: ${message}\n${stack ?? ""}`, true);
  parentPort?.postMessage({
    type: "phash-error",
    taskId: "unknown",
    payload: { message: `Unhandled rejection: ${message}`, stack },
  });
});

sendLog(`Worker process started and ready.`);
...
async function handleMessage(message: unknown): Promise<void> {
  try {
    ...
    if (type !== "compute-phash") {
      console.warn(`[PhashWorker] Received unknown message type: ${type}`);
      return;
    }

    const videoName = payload.videoPath.split(/[/\\]/).pop() || "unknown";
    console.log(`[PhashWorker] [${taskId}] Starting phash computation for: ${videoName}`);

    const { ffmpegPath, videoPath, range, options } = payload;

    sendLog(`[${taskId}] Starting phash computation for: ${videoName}`);

    sendLog(`[${taskId}] Extracting BMP with ffmpeg...`);
    const spriteBmp = await extractSpriteBmp(ffmpegPath, videoPath, range, options);
    sendLog(`[${taskId}] BMP extracted (${spriteBmp.length} bytes)`);

    sendLog(`[${taskId}] Decoding BMP frames...`);
    const sprite = decodeBmpFrame(spriteBmp);

    sendLog(`[${taskId}] Computing pHash fingerprint...`);
    const phash = generateSpritePhashHex(sprite);

    sendLog(`[${taskId}] Successfully computed phash for: ${videoName}`);
    ...
    const taskId = (message as { taskId?: string })?.taskId ?? "unknown";

    sendLog(`[${taskId}] Error computing phash: ${errorMessage}`, true);
    if (stack) {
      sendLog(stack, true);
    }
